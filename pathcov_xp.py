...
if len( sys.argv ) < 1:
	raise Exception( "Usage: pathcov_xp.py [perc=%g] [prob=%g] < <pathways.txt>" %
		(c_dPerc, c_dProb) )
dPerc = float(sys.argv[1]) if ( len( sys.argv ) > 1 ) else c_dPerc
dProb = float(sys.argv[2]) if ( len( sys.argv ) > 2 ) else c_dProb
