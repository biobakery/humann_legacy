...
def readSample( infile ):
    ...
    for rawline in f:
        ...
        if line[0] != "#":
            ...
            if len(pieces)!= 2:
                print "File :"+infile+"\nLine :"+line
                print "More than two elements in this line!"
                exit(-1)
            else:
...
def dictToList( inDict ):
    """Takes a countDict and unrolls it"""
    outList = []
    for key in inDict:
        if inDict[key] > 0:
            for i in range( inDict[key] ):
                outList.append( key )
    return outList
def sample( inList, ss ):
    ...
    for j in range(ntimes):
        thisS = random.sample( inList, xSize )
        theSample += thisS 
    thisS = random.sample( inList, remind )
    theSample += thisS 
    return theSample
def sampleMix( inList1, inList2, sampleSize ):
    xSize = 5
    theSample = []
    theSides = []
    ntimes = int(sampleSize/xSize)
    remind = sampleSize % xSize
    for element in range( 2*sampleSize ):
        theSides.append( element % 2 )
    theSides = random.sample( theSides, sampleSize)
    for i in range(ntimes):
        if theSides[i] == 0:
            thisS = random.sample( inList1, xSize )
        else:
            thisS = random.sample( inList2, xSize )
        theSample += thisS
    if theSides[ntimes] == 0:
        thisS = random.sample( inList1, remind )
    else:
        thisS = random.sample( inList2, remind )
    theSample += thisS 
    return theSample
...
def mainStep1( file1, file2, sampleSize, nreps ):
    sample1 = readSample( file1 )
    sample2 = readSample( file2 )
    sample1h,sample2h = homogenizeDicts( sample1, sample2)
    allKeys = sample1h.keys()

    list1 = dictToList( sample1 )
    list2 = dictToList( sample2 )
    listDeltas = []
    baseDeltas = []
    for i in range(nreps):
        list1x = sample( list1, sampleSize )
        list2x = sample( list2, sampleSize )
        ...
        b1x = sampleMix( list1, list2, sampleSize )
        b2x = sampleMix( list2, list1, sampleSize )
    ...
    for key in allKeys:
        ...
        thisMedian = []
        for i in range( nreps ):
            thisDict = copy.deepcopy(baseDeltas[i])
            thisSpace.append( thisDict[key ] )
            thisDict = copy.deepcopy(listDeltas[i])
            thisMedian.append( thisDict[key] )
...
def doOneCase(file1, file2, outfile, sampleSize=1000,nreps=2000, paranoid=1):
    ...
    for i in range(ntries):
        result = mainStep1( file1, file2, sampleSize, nreps )
        for key in result:
...
if __name__=="__main__":
    ...
    parser.add_option("--samplesize", dest="sampleSize",
           default=1000,
           help="sample size (defaults to 1000), NUMBER", metavar="NUMBER" )
    parser.add_option("--nrepetitions", dest="nreps",
           default=2000,
           help="number of repetitions (defaults to 2000), NUMBER", metavar="NUMBER")
    ...
    if options.filename1 == None:
         print( "Missing input file 1" )
         print( "Use help to determine the syntaxis: xipe --help" )
         quit()
    if options.filename2 == None:
         print( "Missing input file 2" )
         print( "Use help to determine the syntaxis: xipe --help" )
         quit()
